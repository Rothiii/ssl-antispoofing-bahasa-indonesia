...
from model_without_sa import Model
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--database_path",
        type=str,
        default="/media/dl-1/Second Drive/Experiment/Rafid/Dataset/LA/",
    ...
    # Hyperparameters
    parser.add_argument(
    "--models_folder", type=str, help="Folder yang berisi model-model yang sudah dilatih"
    )
            ...
            if args.eval:
                ...
                file_eval = genSpoof_list(
                    dir_meta=os.path.join(
                        args.protocols_path,
                        "{}_cm_protocols/{}.cm.eval.trl.txt".format(prefix, prefix_2019)
                    ),
                    is_train=False,
                    is_eval=True,
                )
                ...
                print(f"Evaluasi selesai untuk model: {model_file}, hasil disimpan di {eval_output_path}")
        sys.exit(0)
    ...
    # List untuk menyimpan loss pada setiap epoch
    epoch_losses = []
    val_losses = []
    ...
    for epoch in range(num_epochs):
        ...
        # Simpan nilai loss ke dalam list
    ...
    # Setelah training selesai, simpan data ke Excel
    df = pd.DataFrame({
    ...
    # Tentukan path penyimpanan Excel
    excel_path = os.path.join(model_save_path, 'losses.xlsx')
